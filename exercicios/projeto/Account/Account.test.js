...
describe('Teste da classe Account', () => {
  ...
  test('sacar com valor de 200 reais ', () => {
  ...
  test('criar conta com dados válidos ', () => {
    // numero conta (5 dígitos), agencia (4 dígitos) e saldo ( número positivo)
    const account = new Account();
  ...
  // criar chave pix inválida
  test('criar chave pix cpf inválida ', () => {
    const account = new Account();
    expect(() => account.createPixKey('3776151406', 'CPF')).toThrow(
      'Erro, cpf inválido'
    );
  });
